...
class StringBuilderParser < RLTK::Parser
  ...
  production(:simple_instruction) do
    ...
    clause('BEGIN instruction END') {|_, n, _| n}
  ...
  production(:bool_expression) do
    clause('TRUE_BOOL')  { |_| BoolExpression.new(:not, :false) }
    clause('FALSE_BOOL') {|_|  BoolExpression.new(:not, :true) }
    clause('LPAREN bool_expression RPAREN') { |_, n, _| n }
    clause('NOT bool_expression') {|n, exp| BoolOperation.new(:not, exp) }
    clause('bool_expression bool_op bool_expression') { |exp1, op, exp2| BoolOperation.new(op, exp1, exp2) }
